...
test("detector requires qualifying messages in two channels inside two minutes", async () => {
  let now = 1_000_000;
  const enforced = [];
  const reports = [];
  const logs = [];
  ...
  assert.match(logs[1], /Enforced guild=guild-1 user=user-1 timeout=true deleted=4\/4/);
